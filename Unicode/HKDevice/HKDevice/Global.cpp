...
//导入设备SDK lib
#pragma comment(lib, "../../HKSDK/HCNetSDK.lib")
#pragma comment(lib, "../../HKSDK/PlayCtrl.lib")
#pragma comment(lib, "../../HKSDK/ShowRemConfig.lib")

//-------------------------------------------------------------------------------------
//        全局变量
//-------------------------------------------------------------------------------------

CRvuManager g_rvuMgr;//全局设备管理对象
...
CRvu::CRvu()
{
	...
	l232Handle		= DFJK_INVALID_HANDLE;
...
//根据某通道实例，获取所属Rvu实例
CRvu* CRvuManager::GetRvuByChan(CChannel* pChan)		
{
	if (pChan == NULL)
		return NULL;
...
//根据lserialHandle获取RVU句柄
CRvu* CRvuManager::GetRvuBySerial(LONG lSerial, int & nType)		
{
	if (lSerial < 0)
		return NULL;
	...
	for (short i=0; i<m_arrayRvu.GetSize(); i++)
	{
		...
		if (pRvu)
		{
			for (short j=0; j<pRvu->m_arrayChan.GetSize(); j++)
			{
				if (pRvu->l232Handle == lSerial)
				{
					nType = 1;
					return pRvu;
				}
				if (pRvu->l485Handle == lSerial)
				{
					nType = 2;
					return pRvu;
				}
