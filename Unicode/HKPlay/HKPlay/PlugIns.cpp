...
//防止DLL被修改（防止用二进制编辑器改DLL内容）※ 只在Release版有效！
//(1) 加入下面的两处#ifndef _DEBUG内的部分
//(2) 在工程Settings→link页面：Customize的Project Options中添加 /release

// #ifndef _DEBUG
// #include <imagehlp.h>	//for MapFileAndCheckSum
...
// #ifndef _DEBUG	//只对Release版有效！
// 	//测试EXE文件本身是否被改变
// 	DWORD HeadChksum=1, Chksum=0;
// 	if (MapFileAndCheckSum(szModName, &HeadChksum, &Chksum) != CHECKSUM_SUCCESS)
// 	{
// 		MessageBox(NULL,"Check fail!","Error", MB_OK);	//函数失败
// 	}
// 	else
// 	{
// 		if (HeadChksum != Chksum)
// 			MessageBox(NULL, "文件被改变！", "Warning", MB_OK);
// 	}
// #endif
